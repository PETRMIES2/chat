...
public class EventChatSubscription extends Subscriber<SopeSocketMessage> {
    private EventFragment fragment;

    public EventChatSubscription(EventFragment fragment) {
    ...
    public void onNext(SopeSocketMessage socketMessage) {
        if (socketMessage.messageType.isGetEventChats() && ChatPagination.hasChats(socketMessage.chatList)) {
